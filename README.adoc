...
To automate domain configuration, a CloudFlare **account-owned** API token is needed
(Manage Account → API Tokens, not My Profile), with `Edit` on all zones in the account:

- `Zone.Zone`
- `Zone.Zone Settings` (Always Use HTTPS, minimum TLS 1.2)
- `Zone.DNS` (records, including CAA / TXT / MX, and DNSSEC)
- `Zone.Single Redirect` (auxiliary domains and Google-hosted apex 301s)

`DnsZone` then manages:

- GitHub Pages: orange-cloud CNAME `@` → `www` → `*.github.io` and Always Use HTTPS
- Google Sites (`ghs.googlehosted.com`): orange-cloud dummy A on `@`, Single Redirect to `www`, `www` DNS-only
- Auxiliary (`DnsZone.alias`): dummy A on `@`/`www`, Single Redirect to the canonical host
- CAA `issue` / `issuewild` for `pki.goog` and `letsencrypt.org` (Universal SSL, Google, GitHub Pages)
- DNSSEC on (Cloudflare is the registrar, so DS at the registry is automatic)
- Workspace mail: `smtp.google.com`, SPF, DKIM, DMARC `p=none`
- Names with no mail: SPF `-all` and DMARC `p=reject`
...
To find out CloudFlare account id, copy it using the
